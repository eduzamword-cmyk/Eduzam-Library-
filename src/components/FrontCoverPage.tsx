...
import defaultCoverBg from '../assets/images/graduation_caps_sky_1786586031603.jpg';
...
export default function FrontCoverPage({ onEnter, onLessonPlanner }: FrontCoverPageProps) {
  ...
  const [coverBg, setCoverBg] = useState<string | null>(() => localStorage.getItem('eduzam_cover_bg'));
  ...
  // Background Image Upload Handlers (supports file picker and drag & drop)
  const handleBgFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    processImageFile(file, (dataUrl) => {
      setCoverBg(dataUrl);
      try {
        localStorage.setItem('eduzam_cover_bg', dataUrl);
      } catch (err) {
  ...
  const handleResetBg = () => {
    setCoverBg(null);
    localStorage.removeItem('eduzam_cover_bg');
  };
  ...
  const activeBg = coverBg || defaultCoverBg;
  ...
  return (
    ...
    >
      {/* Hidden File Inputs for Background and Top-Left Logo */}
      <input 
        ref={bgInputRef}
        type="file" 
        accept="image/*" 
      ...
      {/* Background Image Layer (30% transparent overlay so image is vividly seen) */}
      <div 
      ...
      <header className="w-full max-w-6xl mx-auto flex items-center justify-between z-20 gap-4 flex-wrap">
        {/* Top-Left: Proudly Zambian + Custom Logo Upload */}
        <div className="flex items-center gap-3 group/logo">
          <div className="relative">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-700 p-0.5 shadow-lg shadow-emerald-950/60 flex items-center justify-center overflow-hidden">
              {coverLogo ? (
                <img 
                  src={coverLogo} 
                  alt="Proudly Zambian Emblem" 
                  className="w-full h-full object-cover rounded-[14px]"
                  referrerPolicy="no-referrer"
                />
              ) : (
                <div className="w-full h-full bg-slate-900/95 rounded-[14px] flex items-center justify-center">
            ...
            <button
              ...
              title="Upload Custom Top-Left Logo / Emblem"
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm sm:text-base font-black tracking-wider text-emerald-400 uppercase drop-shadow-sm">
                Proudly Zambian
              </span>
              {coverLogo && (
                <button
        ...
        {/* Right Controls: Audio + Background Upload */}
        <div className="flex items-center gap-3">
          {/* Background Upload / Customize Button */}
          <button
            onClick={() => bgInputRef.current?.click()}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-900/60 hover:bg-slate-900/80 border border-white/10 hover:border-emerald-500/50 text-xs font-bold text-slate-200 transition-all cursor-pointer backdrop-blur-md shadow-xs"
            title="Upload Custom Background Image (or Drag & Drop anywhere)"
          >
            <ImageIcon className="w-3.5 h-3.5 text-emerald-400" />
            <span className="hidden sm:inline">{coverBg ? 'Change Background' : 'Upload Background'}</span>
          </button>

          {coverBg && (
            <button
              onClick={handleResetBg}
              className="p-2 rounded-xl bg-slate-900/60 hover:bg-rose-950/60 border border-white/10 hover:border-rose-500/50 text-slate-300 hover:text-rose-300 transition-all cursor-pointer backdrop-blur-md"
      ...
      <main className="w-full max-w-5xl mx-auto my-auto py-8 sm:py-12 z-20 flex flex-col items-center text-center">
        ...
        <motion.div 
          ...
          className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 mt-8 sm:mt-12 w-full sm:w-auto"
        >
      ...
      <footer className="w-full max-w-6xl mx-auto pt-6 border-t border-white/15 flex flex-col gap-4 z-20">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-300 pb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200">Proudly Zambian • EDUZAM</span>
          </div>
